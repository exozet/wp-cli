...
class Post_Command extends \WP_CLI\CommandWithDBObject
{
    ...
    public function create($args, $assoc_args)
    {
        ...
        if (isset($assoc_args['post_parent']) && !is_int($assoc_args['post_parent'])) {
            $query = new WP_Query(['pagename' => $assoc_args['post_parent'], 'post_type' => ['post', 'page']]);
            $posts = $query->get_posts();
            if (!empty($posts)) {
                $post = $posts[0];
                $assoc_args['post_parent'] = $post->ID;
            }
        }
